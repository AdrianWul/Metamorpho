...
interface SkeletonShape {
  ...
  cornerRadius: number;
  name?: string;
  groupId?: string; // For grouping pills from same text node
}
...
interface TransformOptions {
  ...
  createOnTop: boolean;
  multiplePills: boolean; // Enable multiple random pills per line
}
...
let currentOptions: TransformOptions = {
  heightAdjust: 0,
  pillColor: null,
  createOnTop: false,
...
async function createFlatSkeleton(node: SceneNode): Promise<SceneNode | null> {
  ...
  const skeletonShapes: SkeletonShape[] = [];
  const clonedNodes: { node: SceneNode, x: number, y: number }[] = [];
  ...
  const createdRects: Map<string, RectangleNode[]> = new Map();
...
async function collectElements(
  ...
  containerX: number,
  containerY: number,
  skeletonShapes: SkeletonShape[],
  clonedNodes: { node: SceneNode, x: number, y: number }[]
): Promise<void> {
...
async function handleTextNode(
  ...
  try {
    ...
    const fontName = textNode.fontName as FontName || { family: 'Inter', style: 'Regular' };
    ...
    if (lineHeight === figma.mixed) {
      ...
      lineHeightValue = textNode.fontSize as number * 1.2;
...
function findRootFrame(node: SceneNode): BaseNode | null {
  let current: BaseNode = node;
  let lastFrame: BaseNode | null = null;
...
async function pasteSelection(): Promise<void> {
  ...
  console.log('Destination has auto layout:', 'layoutMode' in destination ? (destination as any).layoutMode : 'N/A');

  const pastedNodes: SceneNode[] = [];

  // Clone and paste each node
  for (const item of copiedNodes) {
    if (!('clone' in item.node)) continue;
...
async function flattenAndMerge(): Promise<void> {
  const selection = figma.currentPage.selection;
...
async function groupByColor(): Promise<void> {
  const selection = figma.currentPage.selection;
...
async function convertFrameToShape(): Promise<void> {
  const selection = figma.currentPage.selection;
  ...
  for (const node of selection) {
    ...
    if ('cornerRadius' in node && typeof node.cornerRadius === 'number') {
      ...
      if (typeof node.topLeftRadius === 'number') shape.topLeftRadius = node.topLeftRadius;
      if (typeof node.topRightRadius === 'number') shape.topRightRadius = node.topRightRadius;
      if (typeof node.bottomLeftRadius === 'number') shape.bottomLeftRadius = node.bottomLeftRadius;
      if (typeof node.bottomRightRadius === 'number') shape.bottomRightRadius = node.bottomRightRadius;
    }
...
function isShape(node: SceneNode): node is RectangleNode | EllipseNode | PolygonNode | StarNode | VectorNode {
